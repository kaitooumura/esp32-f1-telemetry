// --- INÍCIO de src/main.cpp (V15 - FINAL COM CORREÇÃO DRS/RPM) ---
...
struct SharedTelemetryData {
    ...
    // uint8_t  revLightsPercent; // Removido - não é mais usado
        ...
        if (packetId == 2 || packetId == 6 || packetId == 7 || packetId == 12) {
            ...
            if (xSemaphoreTake(g_TelemetryMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
                ...
                if (packetId == 2) { 
                    ...
                    g_Telemetry.lastRevLightsTimestamp = millis(); // Salva o carimbo de data/hora
...
void vTask_DisplayOLED(void *pvParameters) {
    ...
    while (1) {
        ...
        // --- Parte 2: Renderização (V15 - Correção DRS/RPM) ---
        ...
        // --- INÍCIO DA LÓGICA DE INVERSÃO CORRIGIDA ---
        // 1. Verifica se o dado é "novo" (menos de 100ms de idade)
        unsigned long age = millis() - localTelemetry.lastRevLightsTimestamp;
        // 2. Verifica se a luz está acesa E se o RPM está alto (para ignorar o aviso de DRS)
...
void setup() {
    ...
    Serial.begin(115200); 
    Serial.println("Iniciando Sistema de Telemetria F1 RTOS (v15 - DRS Fix)...");
